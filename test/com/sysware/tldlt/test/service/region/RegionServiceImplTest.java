...
/**
 * Type：DeviceTypeServiceImplTest Descript：RegionServiceImpl测试类.
 * Create：SW-ITS-HHE Create Time：2015年7月31日 上午11:02:33 Version：@version
 */
public class RegionServiceImplTest {
	...
	private Dto createInDto(int id, String name, int parentId, String departmentId) {
	...
	private void mockQueryDepartmentId(String departmentId) {
		...
		Mockito.when(organizationService.queryDeptinfoByDeptid(Mockito.any(Dto.class))).thenReturn(departmentDto);
	}
	...
	private void mockQueryRegionId(int regionId) {
		Mockito.when(appDao.queryForObject("App.Region.queryRegionInfoById", regionId))
				.thenReturn(createInDto(regionId, "aaa", 0, "001"));
	}
	...
	private void mockQueryChildRegionId(int regionId) {
		Mockito.when(appDao.queryForList("App.Region.queryChildRegionIds", regionId))
	...
	public void testAddInfo_Fail_RegionId_3_Exist() {
		Dto inDto = createInDto(3, "2", 1, "00001");
	...
	public void testAddInfo_Fail_DepartmentId_00001_NotExist() {
		...
		Mockito.when(organizationService.queryDeptinfoByDeptid(Mockito.any(Dto.class))).thenReturn(null);
		BaseRetDto outDto = (BaseRetDto) regionServiceImpl.addInfo(inDto);
		assertThat(outDto.getRetCode(), is(AppCommon.RET_CODE_INVALID_VALUE));
	}
	...
	public void testAddInfo_Fail_ParentId_1_NotExist() {
		...
		Mockito.when(appDao.queryForObject("App.Region.queryRegionInfoById", parentId)).thenReturn(null);
		BaseRetDto outDto = (BaseRetDto) regionServiceImpl.addInfo(inDto);
		assertThat(outDto.getRetCode(), is(AppCommon.RET_CODE_INVALID_VALUE));
	}
	...
	public void testQueryRegionItems_Success_ParentId_0() {
		Dto dto = new BaseDto();
		dto.put("parentId", 0);
		List list = Lists.newArrayList(createInDto(1, "test", 0, "001"), createInDto(2, "test1", 0, "002"));
		Mockito.when(appDao.queryForList("App.Region.queryRegionItemsByDto", dto)).thenReturn(list);
	...
	private void mockDeleteRegionId(int regionId) {
		Mockito.when(appDao.delete("App.Region.deleteInfo", regionId)).thenReturn(1);
	}
	...
	public void testDelete_Success_RegionId_15_16() {
		...
		mockQueryChildRegionId(15);
		mockQueryChildRegionId(16);
		mockDeleteRegionId(15);
		mockDeleteRegionId(16);
