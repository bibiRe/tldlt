...
public class ServletContextSimulator implements ServletContext {
    ...
    private static Log logger = LogFactory
            .getLog(ServletContextSimulator.class);
     ...
     *         of the attribute, or <code>null</code> if no attribute exists
     *         matching the given
     *         name
     ...
     * within this servlet context. Use the {@link #getAttribute} method with an
     * attribute name
     ...
     *         containing the names of the context's
     ...
     * <p>
     * In this mock implementation, this method first looks for the supplied
     * pathname in the underlying filesystem; if it does not exist there, the
     * default Java classloader is used.
    ...
    public File getResourceAsFile(String path) {
        ...
        // If the path is relative then apply the contextDirectory path if it
        // exists.
        if (!file.exists()) {
            if (!path.startsWith("/")) {
                path = "/" + path;
     ...
     *       instead.
     *       <p>
     *       This method was originally defined to write an exception's stack
     *       trace and an explanatory error message to the servlet log file.
     */
     ...
     * container notifies them accordingly. @param name a <code>String</code> specifying the name
    ...
    /**
     * Sets the absolute context directory to be used in the getRealPath()
     * method.
     * @param contextDirectory the absolute path of the root context directory
    ...
    public String getContextPath() {
        String result = "";
        if (null != contextDirectory) {
            result = contextDirectory.getAbsolutePath();
        }
        return result;
    }
