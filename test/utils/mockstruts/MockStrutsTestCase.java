...
/**
 * MockStrutsTestCase is an extension of the base JUnit testcase that provides
 * additional methods to aid in testing Struts Action objects. It uses a mock
 * object approach to simulate a servlet container, and tests the execution of
 * Action objects as they are actually run through the Struts ActionServlet.
 * MockStrutsTestCase provides methods that set up the request path, request
 * parameters for ActionForm subclasses, as well as methods that can verify that
 * the correct ActionForward was used and that the proper ActionError messages
 * were supplied. <br>
 * <br>
 * <b>NOTE:</b> By default, the Struts ActionServlet will look for the file
 * <code>WEB-INF/struts-config.xml</code>, so you must place the directory that
 * <i>contains</i> WEB-INF in your CLASSPATH. If you would like to use an
...
public abstract class MockStrutsTestCase extends TestCase {
	...
	/**
	 * Constructor that takes test name parameter, for backwards compatibility
	 * with older versions on JUnit.
	 */
	 ...
	 * Note that if {@link #setRequestWrapper} has not been called, this method
	 * will return an instance of javax.servlet.http.HttpServletRequestWrapper.
	 */
	...
	/**
	 * Set this TestCase to use a given HttpServletRequestWrapper class when
	 * calling Action.execute(). Note that if this method is not called, then
	 * the normal HttpServletRequest object is used.
	 * 
	 * @param wrapper
	 *            an HttpServletRequestWrapper object to be used when calling
	 *            Action.execute().
	 */
	...
	/**
	 * Returns an HttpServletResponseWrapper object that can be used in this
	 * test. Note that if {@link #setResponseWrapper} has not been called, this
	...
	/**
	 * Set this TestCase to use a given HttpServletResponseWrapper class when
	 * calling Action.execute(). Note that if this method is not called, then
	 * the normal HttpServletResponse object is used.
	 * 
	 * @param wrapper
	 *            an HttpServletResponseWrapper object to be used when calling
	 *            Action.execute().
	 */
	...
	/**
	 * Returns the mock HttpServletResponse object used in this test. This
	 * allows access to methods for setting up test preconditions that are
	 * otherwise unavailable through the normal Servlet API.
	 */
	...
	/**
	 * Sets the ActionServlet to be used in this test execution. This method
	 * should only be used if you plan to use a customized version different
	 * from that provided in the Struts distribution.
	 */
	 ...
	 *                if there are any execution errors while calling
	 *                Action.execute()
	 */
	...
	public void setRequestParameter(String parameterName, String[] parameterValue) {
		if (logger.isDebugEnabled())
			logger.debug("Entering - parameterName = " + parameterName + ", parameterValue = " + parameterValue);
		init();
		this.request.setParameterValue(parameterName, parameterValue);
		if (logger.isDebugEnabled())
			logger.debug("Exiting");
	}
	 ...
	 * 
	 * @param pathInfo
	 *            the request path to be processed. This should correspond to a
	 *            particular action mapping, as would normally appear in an HTML
	 *            or JSP source file.
	 */
	...
	/**
	 * Sets the request path instructing the ActionServlet to used a particual
	 * ActionMapping. Also sets the ServletPath property on the request.
	 * 
	 * @param moduleName
	 *            the name of the Struts sub-application with which this request
	 *            is associated, or null if it is the default application.
	 * @param pathInfo
	 *            the request path to be processed. This should correspond to a
	 *            particular action mapping, as would normally appear in an HTML
	 *            or JSP source file. If this request is part of a
	 *            sub-application, the module name should not appear in the
	 *            request path.
	 */
	 ...
	 * @param value
	 *            the value of the intialization parameter
	 */
	 ...
	 *            a File object representing the root context directory for this
	 *            application.
	 */
	...
	/**
	 * Sets the location of the Struts configuration file for the default
	 * module. This method can take either an absolute path, or a relative path.
	 * If an absolute path is supplied, the configuration file will be loaded
	 * from the underlying filesystem; otherwise, the ServletContext loader will
	 * be used.
	 */
	 ...
	 * 
	 * @param moduleName
	 *            the name of the sub-application, or null if this is the
	 *            default application
	 * @param pathname
	 *            the location of the configuration file for this
	 *            sub-application
	 */
	...
	public void setServletConfigFile(String pathname) {
		...
		try {
			...
			throw new AssertionFailedError(
					"Received an exception while loading web.xml - " + e.getClass() + " : " + e.getMessage());
		}
	 ...
	 * 
	 * @param forwardName
	 *            the logical name of a forward, as defined in the Struts
	 *            configuration file. This can either refer to a global forward,
	 *            or one local to the ActionMapping.
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller used a different forward
	 *                than <code>forwardName</code> after executing an Action
	 *                object.
	 */
	public void verifyForward(String forwardName) throws AssertionFailedError {
		...
		init();
		String moduleName = (String) this.request.getAttribute(Common.INCLUDE_SERVLET_PATH);
		if (moduleName.endsWith("/")) {
			moduleName = moduleName.substring(0, moduleName.length() - 1);
		}
		if ((null != moduleName) && (null != actionPath)) {
			if (actionPath.startsWith(moduleName)) {
				actionPath = actionPath.substring(moduleName.length());
			}
		}
	 ...
	 * 
	 * @param forwardPath
	 *            an absolute pathname to which the request is to be forwarded.
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller used a different forward
	 *                path than <code>forwardPath</code> after executing an
	 *                Action object.
	 */
	...
	/**
	 * Verifies if the ActionServlet controller forwarded to the defined input
	 * path.
	 * 
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller used a different forward
	 *                than the defined input path after executing an Action
	 *                object.
	 */
	...
	/**
	 * Verifies that the ActionServlet controller used this forward and Tiles
	 * definition.
	 * 
	 * @param forwardName
	 *            the logical name of a forward, as defined in the Struts
	 ...
	 *            the name of a Tiles definition, as defined in the Tiles
	 *            configuration file.
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller used a different forward
	 *                or tiles definition than those given after executing an
	 *                Action object.
	 */
	public void verifyTilesForward(String forwardName, String definitionName) {
		...
		init();
		Common.verifyTilesForward(actionPath, forwardName, definitionName, false, request, config.getServletContext(),
				config);
		if (logger.isTraceEnabled())
			logger.trace("Exiting");
	}
	...
	/**
	 * Verifies that the ActionServlet controller forwarded to the defined input
	 * Tiles definition.
	 * 
	 * @param definitionName
	 *            the name of a Tiles definition, as defined in the Tiles
	...
	/**
	 * Verifies if the ActionServlet controller sent these error messages. There
	 * must be an exact match between the provided error messages, and those
	 * sent by the controller, in both name and number.
	 * 
	 * @param errorNames
	 *            a String array containing the error message keys to be
	 *            verified, as defined in the application resource properties
	 *            file.
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller sent different error
	 *                messages than those in <code>errorNames</code> after
	 ...
	 * 
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller sent any error messages
	 *                after excecuting and Action object.
	 */
	 ...
	 * 
	 * @param messageNames
	 *            a String array containing the action message keys to be
	 *            verified, as defined in the application resource properties
	 *            file.
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller sent different action
	 *                messages than those in <code>messageNames</code> after
	 *                executing an Action object.
	 */
	 ...
	 * 
	 * @exception AssertionFailedError
	 *                if the ActionServlet controller sent any action messages
	 *                after excecuting and Action object.
	 */
	 ...
	 * Note that no form will be returned if the Action being tested cleans up
	 * the form instance. @ return the ActionForm instance used in this test, or
	...
	/**
	 * Sets an ActionForm instance to be used in this test. The given ActionForm
	 * instance will be stored in the scope specified in the Struts
	 * configuration file (ie: request or session). Note that while this
	 * ActionForm instance is passed to the test, Struts will still control how
	 * it is used. In particular, it will call the ActionForm.reset() method, so
	 * if you override this method in your ActionForm subclass, you could
	 * potentially reset attributes in the form passed through this method.
	 * 
	 * @param form
