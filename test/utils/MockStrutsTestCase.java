 ...
 * ActionError messages were supplied. <br>
 * <br>
...
public abstract class MockStrutsTestCase extends TestCase {
        ...
        try {
            ...
            if (e.getMessage().equals("java.lang.NullPointerException")) {
                ...
                if (this.context.getContextDirectory() == null) {
                    ...
                    message = message
                            + "is "
                            + this.context.getContextDirectory()
                                    .getAbsolutePath();
                }
                message = message
                        + ".  /WEB-INF/web.xml must be found under the context directory, "
    ...
    public void actionPerform() {
        ...
        if (!this.requestPathSet) {
            throw new IllegalStateException(
                    "You must call setRequestPathInfo() prior to calling actionPerform().");
        }
        init();
        ...
        try {
            ...
            if (this.context.getContextDirectory() == null) {
                ...
                message = message + "is "
                        + this.context.getContextDirectory().getAbsolutePath();
            }
            message = message
                    + ".  struts config file must be found under the context directory, "
                    + "the directory the test case is running from, or in the classpath.";
     ...
     */
    public void
            addRequestParameter(String parameterName, String parameterValue) {
        if (logger.isDebugEnabled())
            logger.debug("Entering - parameterName = " + parameterName
    ...
    public void setRequestPathInfo(String moduleName, String pathInfo) {
        if (logger.isDebugEnabled())
            logger.debug("Entering - moduleName = " + moduleName
                    + ", pathInfo = " + pathInfo);
        init();
        ...
        if (moduleName != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("setting request attribute - name = "
                        + Common.INCLUDE_SERVLET_PATH + ", value = "
                        + moduleName);
            }
     ...
     * @param moduleName the name of the sub-application, or null if this is the
     *            default application
        ...
        if (response.containsHeader("Location")) {
            ...
            try {
                String strippedForward = request.getContextPath()
                        + Common.stripJSessionID(((ServletContextSimulator) config
                                .getServletContext())
                                .getRequestDispatcherSimulator().getForward());
                if (logger.isDebugEnabled()) {
                    logger.debug("stripped forward and added context path - "
                            + strippedForward);
                }
                if (logger.isDebugEnabled())
                    logger.debug("Exiting");
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "caught NullPointerException - returning null", npe);
                }
                return null;
            }
    }
     ...
     *                used a different forward than <code>forwardName</code>
     *                after
    ...
    public void verifyForward(String forwardName) throws AssertionFailedError {
        ...
        Common.verifyForwardPath(actionPath, forwardName, getActualForward(),
                false, request, config.getServletContext(), config);
        if (logger.isDebugEnabled())
            logger.debug("Exiting");
    }
     ...
     *            is to be forwarded.
     * @exception AssertionFailedError if the ActionServlet controller
     *                used a different forward path than
     *                <code>forwardPath</code> after
     *                executing an Action object.
     */
    public void verifyForwardPath(String forwardPath)
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("expected forward = '" + forwardPath
                    + "' - actual forward = '" + actualForward + "'");
        }
    ...
    public void verifyInputForward() {
        if (logger.isDebugEnabled())
            logger.debug("Entering");
        init();
        Common.verifyForwardPath(actionPath, null, getActualForward(), true,
                request, config.getServletContext(), config);
        if (logger.isDebugEnabled())
            logger.debug("Exiting");
    }
     ...
     * @exception AssertionFailedError if the ActionServlet controller
     *                used a different forward or tiles definition than those
     *                given after
     *                executing an Action object.
     */
    public void verifyTilesForward(String forwardName, String definitionName) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - forwardName=" + forwardName
                    + ", definitionName=" + definitionName);
        init();
        Common.verifyTilesForward(actionPath, forwardName, definitionName,
                false, request, config.getServletContext(), config);
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    }
    ...
    public void verifyInputTilesForward(String definitionName) {
        ...
        init();
        Common.verifyTilesForward(actionPath, null, definitionName, true,
                request, config.getServletContext(), config);
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    }
     ...
     *            to be verified, as defined in the application resource
     *            properties
     *            file.
     * @exception AssertionFailedError if the ActionServlet controller
     *                sent different error messages than those in
     *                <code>errorNames</code> after executing an Action object.
     */
    ...
    public void verifyActionErrors(String[] errorNames) {
        ...
        init();
        Common.verifyActionMessages(request, errorNames, Globals.ERROR_KEY,
                "error");
        if (logger.isDebugEnabled())
            logger.debug("Exiting");
    }
     ...
     * executing an Action object.
     * @exception AssertionFailedError if the ActionServlet controller
     *                sent any error messages after excecuting and Action
     *                object.
     */
     ...
     *            file.
     * @exception AssertionFailedError if the ActionServlet controller
     *                sent different action messages than those in
     *                <code>messageNames</code> after executing an Action
     *                object.
     */
     ...
     * executing an Action object.
     * @exception AssertionFailedError if the ActionServlet controller
     *                sent any action messages after excecuting and Action
     *                object.
     */
