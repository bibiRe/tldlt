...
public class Common {
        ...
        if (messages != null) {
            ...
            if (iterator.hasNext()) {
                ...
                while (iterator.hasNext()) {
                    messageText.append(" \"");
                    messageText.append(((ActionMessage) iterator.next())
                            .getKey());
                    messageText.append("\"");
                }
                throw new AssertionFailedError("was expecting no "
                        + messageLabel + " messages, but received: "
    ...
    protected static void verifyActionMessages(HttpServletRequest request,
            String[] messageNames, String key, String messageLabel) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - request = " + request
                    + ", messageNames = " + messageNames + ", key = " + key
                    + ", messageLabel = " + messageLabel);
        ...
        if (messages == null) {
            throw new AssertionFailedError("was expecting some " + messageLabel
                    + " messages, but received none.");
        }
        /* check length of messages as optimization */
        else if (messages.size() != messageNames.length) {
            throw new AssertionFailedError("was expecting "
                    + messageNames.length + " " + messageLabel
                    + " message(s), but received " + messages.size() + " "
                    + messageLabel + " message(s)");
        } else {
            ...
            String[] messageKeys = (String[]) IteratorUtils.toArray(iter,
                    String.class);
            ...
            for (int i = 0; i < messageNames.length; i++) {
                if (!messageNames[i].equals(messageKeys[i])) {
                    ...
                    throw new AssertionFailedError("received " + messageLabel
                            + " messages: (" + mks + ") but expected (" + mns
                            + ")");
                }
            }
        }
     ...
     * with this tile definition.
     * @throws AssertionFailedError if the expected and actual tiles definitions
     *             do not match.
     */
    @SuppressWarnings("unused")
    protected static void verifyTilesForward(String actionPath,
            String forwardName, String expectedDefinition, boolean isInputPath,
            HttpServletRequest request, ServletContext context,
            ServletConfig config) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - actionPath = " + actionPath
        ...
        if ((forwardName == null) && (isInputPath)) {
            ...
            forwardName = getActionConfig(actionPath, request, context)
                    .getInput();
            if (logger.isDebugEnabled()) {
            ...
            if (forwardName == null)
                throw new AssertionFailedError(
                        "Trying to validate against an input mapping, but none is defined for this Action.");
        }
        ...
        if (!isInputPath) {
            ...
            ForwardConfig expectedForward = findForward(actionPath,
                    forwardName, request, context);
        ...
        if (definitionName == null)
            throw new AssertionFailedError(
                    "Could not find tiles definition mapped to forward '"
                            + forwardName + "'");
        if (!definitionName.equals(expectedDefinition))
            throw new AssertionFailedError("Was expecting tiles definition '"
                    + expectedDefinition + "' but received '" + definitionName
                    + "'");
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    }
     ...
     */
    protected static void verifyForwardPath(String actionPath,
            String forwardName, String actualForwardPath, boolean isInputPath,
            HttpServletRequest request, ServletContext context,
            ServletConfig config) {
        ...
        if (!isInputPath) {
            ...
            if (forwardName == null) {
                if (actualForwardPath == null)
                    ...
                    throw new AssertionFailedError(
                            "Expected a null forward from action, but received '"
                                    + actualForwardPath + "'");
            }
        ...
        String moduleName = request.getServletPath() != null ? request
                .getServletPath() : "";
        ...
        if ((moduleName != null) && (moduleName.length() > 0))
            moduleName = moduleName.substring(moduleName.indexOf('/'),
        ...
        if (usesTiles)
            ...
            forwardName = request.getContextPath() + forwardName;
        if (logger.isDebugEnabled()) {
            logger.debug("added context path and module name to forward = "
                    + forwardName);
        }
        if (actualForwardPath == null) {
            if (logger.isDebugEnabled()) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("expected forward = '" + forwardName
                    + "' - actual forward = '" + actualForwardPath + "'");
        }
        if (!forwardName.equals(stripJSessionID(actualForwardPath)))
            throw new AssertionFailedError("was expecting '" + forwardName
                    + "' but received '" + actualForwardPath + "'");
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    }
    ...
    /**
     * Returns any ActionForm instance stored in the request or session, if
     * available.
     */
    protected static ActionForm getActionForm(String actionPath,
            HttpServletRequest request, ServletContext context) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - actionPath = " + actionPath
                    + ", request = " + request + ", context = " + context);
        ActionForm form;
        ActionConfig actionConfig = getActionConfig(actionPath, request,
                context);
        if ("request".equals(actionConfig.getScope())) {
            ...
            form = (ActionForm) session.getAttribute(actionConfig
                    .getAttribute());
        }
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    ...
    /**
     * Returns a ForwardConfig for the given forward name. This method first
     * searches for the forward in the supplied
     * action mapping. If it is not defined there, or if the mapping is not
     * provided, it searches for it globally.
     */
    protected static ForwardConfig findForward(String mappingName,
            String forwardName, HttpServletRequest request,
            ServletContext context) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - mappingName = " + mappingName
                    + ", forwardName = " + forwardName + ", request = "
                    + request + ", context = " + context);
        ForwardConfig forward = null;
        // first, look for forward in mapping (if it's defined)
        if (mappingName != null) {
            ActionConfig mapping = getActionConfig(mappingName, request,
                    context);
            forward = mapping == null ? null : mapping
     ...
     */
    protected static ActionConfig getActionConfig(String mappingName,
            HttpServletRequest request, ServletContext context) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - mappingName = " + mappingName
                    + ", request = " + request + ", context = " + context);
        ModuleConfig config = getModuleConfig(request, context);
        ActionMapping actionMapping = (ActionMapping) config
    ...
    protected static ModuleConfig getModuleConfig(HttpServletRequest request,
            ServletContext context) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - request = " + request + ", context = "
                    + context);
        if (logger.isDebugEnabled()) {
    ...
    protected static void setActionForm(ActionForm form,
            ...
            ServletContext context) {
        if (logger.isTraceEnabled())
            logger.trace("Entering - form = " + form + ", request = " + request
                    + ", actionPath = " + actionPath + ", context = " + context);

        if (actionPath == null || actionPath.equalsIgnoreCase(""))
            throw new IllegalStateException(
                    "You must call setRequestPathInfo() before calling setActionForm()!");
        ...
        if (actionConfig.getScope().equals("request")) {
            ...
            request.getSession()
                    .setAttribute(actionConfig.getAttribute(), form);
        }
        if (logger.isTraceEnabled())
            logger.trace("Exiting");
    }
